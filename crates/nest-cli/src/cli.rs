...
/// `.nest` file and (where relevant) a query VECTOR; they never run python.
...
#[command(version)]
#[command(
    about = "nest: single-file, memory-mapped, hash-verified vector database with stable citations",
    long_about = None,
...
pub enum Commands {
    ...
    /// [engine] Search a `.nest` file with a JSON-array query vector (exact path).
    ...
    Stats { file: PathBuf },
    /// [engine] Resolve a `nest://content_hash/chunk_id` citation into the
