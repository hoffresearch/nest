...
//! build is not timed). run: `cargo bench -p nest-runtime --bench rerank`
...
use nest_runtime::ann::{DEFAULT_M, HnswIndex};
...
fn build(path: &PathBuf, vectors: &[f32], hnsw: &[u8], dtype: EmbeddingDType) {
    ...
    let mut b = NestFileBuilder::new(manifest).embedding_dtype(dtype);
    for i in 0..N {
...
fn bench_search(c: &mut Criterion) {
    ...
    for (name, dtype) in dtypes {
        let path = dir.join(format!("nest-bench-rerank-{name}.nest"));
        build(&path, &vectors, &hnsw, dtype);
        let file = MmapNestFile::open(&path).expect("open bench corpus");
        let mut qi = 0usize;
