...
|---|---|---|
| `nest-view` | `NestView::from_bytes` + every section decoder + hashes | reader / decoder bug |
...
| `mmap-open-search` | file on disk, `MmapNestFile::open` + every search verb | runtime open / search bug |

`nest-view` and `mmap-open-search` reseal half of their inputs (recompute
...
cd fuzz
mkdir -p corpus/nest-view && cp seeds/*.bin ../crates/nest-format/tests/fixtures/golden_v1_minimal.nest corpus/nest-view/
cargo +nightly fuzz run nest-view -- -max_total_time=600
