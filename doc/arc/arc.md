...
- crates/nest-cli/src/main.rs | rust source | clap entrypoint that wires nine subcommands to the corresponding cmd modules.
...
- crates/nest-python/src/nest_file.rs | rust source | wraps MmapNestFile for python and re-exposes search, search_ann, search_graph, search_hybrid, inspect, validate, and metadata getters (incl has_graph).
...
- crates/nest-runtime/src/lib.rs | rust source | public SearchHit and SearchResult types plus runtime re-exports (incl the graph module).
...
- crates/nest-runtime/src/search.rs | rust source | validates queries, scores exact rows and reranks ann, graph, and hybrid candidates through the single rerank source, and materializes stable hit contracts; search_graph seeds the exact top-ef then bounded-bfs-expands the union before the same exact rerank.
