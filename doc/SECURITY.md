...
- a path that bypasses `model_hash` validation in any query path (`search-text`, `ask`, `retrieve`, `search-space`) without the user passing an explicit skip flag
- a path that executes model-repo code (`trust_remote_code` presets) without the spec's `allow_remote_code` opt-in, or with a code file whose sha256 is outside the pinned allowlist in `python/forge/model_registry.py`
