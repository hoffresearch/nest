...
- corpus licensing: a distributed `.nest` inherits the license of the content it embeds. the demo corpus that ships with nest is a union of upstream datasets with different licenses (some cc-by-sa, some unspecified academic distributions); the per-source bill of materials and the effective redistribution obligations are in [`dat/demo/Instructions.md`](../dat/demo/Instructions.md#corpus-license-bill-of-materials). the repository code is mit; that does not cover corpus content. redistributing a built `.nest` means honoring the most restrictive upstream license and carrying attribution. for anything shipped broadly, prefer a permissive or cc0 corpus.
