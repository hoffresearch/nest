...
- hnsw build 1.73x faster on 20k x 384 (hardening-plan §4.11), still single-threaded and byte-deterministic across machines: the build distance is an eight-accumulator fixed-tree dot the compiler vectorizes without fma on every target, and the build reuses one epoch-stamped visited list instead of allocating a hash set per layer search. the graph bytes are pinned by a test for two seeded builds; the 10k digest moved once, on purpose, with the reason recorded in the test.
- nest-format under miri (hardening-plan §4.4): the nightly ci runs `cargo miri test -p nest-format` (tree borrows; tests that need zstd's c code, `half`'s aarch64 inline asm or the fsst table build are marked in the source with the reason). the first run found a stacked-borrows report inside `sha2` 0.11's neon backend (dependency, tree-borrows clean), and a deliberate `'static` leak in a test helper, now removed.
- hnsw build 1.73x faster on 20k x 384 (hardening-plan §4.11), still single-threaded and byte-deterministic across machines: the build distance is an eight-accumulator fixed-tree dot the compiler vectorizes without fma on every target, and the build reuses one epoch-stamped visited list instead of allocating a hash set per layer search. the graph bytes are pinned by a test for two seeded builds; the 10k digest moved once, on purpose, with the reason recorded in the test. `doc/benchmarks.md` re-run on the same 100k x 384 rows: nest hybrid build 224 s -> 173 s (hnswlib 83 s, usearch 109 s), p50 0.94 -> 0.72 ms, cold open 309 -> 292 ms, bytes unchanged.
...
- property tests for the section codecs (hardening-plan §4.5): `crates/nest-format/tests/codec_roundtrip_prop.rs` checks, over generated inputs, that intpack, txt_streams, fsst, zstd_dict and dedup decode to the raw payload byte for byte, that float16/int8/int4 reconstruct within their quantization step and are sized exactly as `expected_embeddings_size` says, and that every encoder is a pure function of its input (two calls, identical bytes). `proptest` is a dev-dependency of nest-format only.
- criterion benches in nest-runtime (hardening-plan §4.7): `cargo bench -p nest-runtime` runs the simd kernels per dtype and dim on the detected backend, the public `search` / `search_ann` path end to end over one 20k-row `.nest` per stored dtype, and the hnsw build at the knobs `doc/benchmarks.md` uses; ci compiles them on every pull request.
- `doc/install.md`: every install channel (one-liner, windows, `nestdb` wheel, homebrew tap, cargo binstall, docker, dev build), what each lays down and where, verification (per-file sha256, sigstore attestations, the post-release install-test), offline and air-gapped notes, and the maintainer one-time checklist. the file was linked from six places and never existed. the checklist records what the release workflows assume but does not exist yet (verified 2026-09-10): the `hoffresearch/homebrew-nest` tap and its token (the `publish-homebrew-formula` job fails without them), the pypi trusted publisher and `pypi` environment for `nestdb`, and the `get.hoffresearch.com` short url (nxdomain). `v0.3.0` predates the pipeline and carries no artifacts, so the channels serve from the first tag cut after it.
...
- the stray `l` prefix on 26 forge-core doc comments that the earlier sweep missed (four lowercase-initial: `/// lin the default`, `/// lfield order`, ...), and the `space[0]` rustdoc unresolved-link warning in `embedding_request.rs`. stale paths: `doc/arc/arc.yaml` named `crates/nest-runtime/src/ann/codec.rs` (split into `codec/mod.rs` + `codec/tests.rs` in #71) and `.github/workflows/dist-build-setup.yml` (moved to `.github/` in 723ba186); `llms.txt` named `dat/demo/README.md` (renamed to `Instructions.md` in #99); a nest-format module comment cited the old codec path. `.contracts/.agents/AGENTS.md` called `v0.2.0` the current tag and described README sections that no longer exist. arc.yaml counted nine engine subcommands where the cli has twelve plus three agent verbs, and its inventory lacked `doc/usage.md`, `doc/changelog.md`, `doc/data-governance.md` and `doc/install.md`.
...
- contributor and agent docs describe the branch model actually in use: `main` is the only long-lived branch, work happens on short-lived branches off it, and every change lands as a squash-merged pull request under the `main` ruleset (pull request required, verified ssh-signed commits, linear history); delete the branch after merge. `.contracts/.agents/AGENTS.md`, `doc/CONTRIBUTING.md` and `llms.txt` previously told contributors to branch from and target a `dev` integration branch that does not exist on the remote. the installer headers (`scripts/install.sh`, `scripts/install.ps1`) show the raw github one-liner the README uses instead of the unregistered short url, and the README install section says that `v0.3.0` predates the release pipeline.
