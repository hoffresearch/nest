...
metadata filter (`0x17`) + `search_filtered`, build-time chunk dedup, and PHI-safety hardening (all additive within v1, no format-version bump):

- `meta_index` section (`0x17`, `META_INDEX_PAYLOAD_VERSION=1`): a generic `(field, value) -> sorted chunk ordinals` inverted index; delta-gapped postings in one shared intpack column (the bm25 codec shape). EXCLUDED from `content_hash` (like bm25/hnsw/graph 0x0C) and opened by section presence with NO manifest flag, so attaching it leaves a `nest://` citation byte-identical (regression: `meta_index_content_hash.rs`). `field`/`value` are arbitrary caller strings — no market rule lives in nest. The decode path is treated as a hostile-input trust boundary: typed errors on truncation/version/EOF, reservation bounded by remaining bytes (no allocator abort), and postings rejected unless strictly ascending (no duplicate-citation from a corrupt file).
- runtime `MmapNestFile::search_filtered(query, field, value, k)` (+ python `NestFile.search_filtered`, cli `nest search-filtered`): restricts the EXACT cosine to the `(field, value)` chunk subset; recall is 1.0 WITHIN the filter, and an absent field/value or a no-meta_index file returns an honest EMPTY result (NaN recall, never a silent whole-corpus fallback; regression: `search_filtered.rs`). Measured on the clinical cohort, scoping by patient moved group-recall@10 from 0.092 to 0.999 over the in-group subset.
- `build_from_zip --index-fields F1,F2` emits the 0x17 index from per-doc labels. `--dedup` (default on) drops exact-duplicate + empty chunks before indexing, but is SCOPED PER DOC whenever `--index-fields` is set, so dedup never crosses a meta_index entity (a chunk shared by two notes stays in BOTH postings with correct provenance; global first-doc dedup would have dropped 400 chunk-instances from 102 patients' postings on the cohort).
- PHI-safety: `.gitignore` ignores all `*.nest` / `*.sidecar.jsonl` / `**/pairs/` (negating only the three public corpora) so a clinical artifact survives even a `git add -A --no-verify`; `scripts/pre-commit` aborts a forced stage; `doc/phi-safety.md` records the threat model (clinical data lives outside any git repo) and the three layered controls.
- section-id map: `meta_index` takes `0x17` in the reconciled additive band (joining 0x09-0x16); it stays out of CANONICAL_SECTIONS so it never moves a citation.
